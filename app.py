...
if df is not None:
    ...
    if task == "Data Exploration":
        ...
        choice = st.sidebar.radio("",["columns convert the numerical form","Feature Selection", "Filter Data"])
        if choice == "columns convert the numerial form":
            st.subheader("columns conver the numerical ")
            if df is not None:
             df = pd.read_csv(df)
             st.write(df)
...
#profit = ProfileReport(df) 

      #      st.write(profit.to_html())
